...
class SimpleMonitor13(switchm.SimpleSwitch13):
    ...
    def write_report(self, attack_details):
    ...
    def flow_predict(self):
        try:
            ...
            # Removed unnecessary str.replace calls
            ...
            for i in range(len(y_flow_pred)):  # Using index-based iteration
                if y_flow_pred[i] == 0:
                    legitimate_trafic += 1
                else:
                    ddos_trafic += 1
                    victim = int(predict_flow_dataset.iloc[i, 5].replace('.', '')) % 20  # Corrected IP conversion
                    attack_detail = {
            ...
            if (legitimate_trafic / len(y_flow_pred) * 100) > 80:
                ...
                self.mitigation = 1
                threading.Thread(target=self.write_report, args=(attack_details,)).start()
